...
title "Checking out to the target branch"
...
title "Getting current values and the time spent..."
for f in "${maps_arg[@]}"; do
    echo "Running test $iter"
    for e in "${cmd_arr[@]}"; do
        curr_arg="${e} ${f}"
        eval target/release/logram "$(echo $curr_arg)" >$student_dir/$iter.txt
        result=$(hyperfine -w 5 -i -N "target/release/logram < $curr_arg")
        mean=$(echo "$result" | awk '/Time \(mean ± σ\):/ {print $5}')
        student_result+=($mean)
        iter=$((iter + 1))
    done
done
...
title "Building original code"
cargo build --release

title "Getting correct values and the original time spent..."
for f in "${maps_arg[@]}"; do
    for e in "${cmd_arr[@]}"; do
        curr_arg="${e} ${f}"
